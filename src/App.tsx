...
export default function App() {
  ...
  const handleVerifyKey = async () => {
    ...
    try {
      ...
      setIsKeyVerified(false);
      setVerificationMessage({ type: "error", text: "네트워크 오류가 발생했습니다. 잠시 후 다시 시도해 주세요." });
    } finally {
  ...
  const triggerAnalysis = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("서버에서 기술 검토를 처리하지 못했습니다.");
      }
      ...
      console.error(err);
      setErrorMessage(err.message || "안전 기술 검토 도중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.");
    } finally {
      setIsLoading(false);
    }
  };
